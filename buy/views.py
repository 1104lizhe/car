...
def add_order(request):
    if request.user.is_authenticated():
        ...
        try:
            ...
            brand = str(car_.serbran) + car_.c_title
            picture = car_.picture
            price = car_.extractprice
            newprice = car_.newprice
            mileage = car_.mileage
            Cart.objects.create(susers=request.user, car=car_, brand=brand, picture=picture, price=price, newprice=newprice, mileage=mileage)
...
def confirmbuy(request):
    if request.user.is_authenticated():
        ...
        try:
            ...
            Orders.objects.create(sale_users=car[0].user, buy_users=request.user, brand=brand, picture=picture, price=price, newprice=newprice, mileage=mileage, orderNo=orderNo)
        ...
        orders = Orders.objects.filter(buy_users=user_id).order_by("-id")[:4]
